  ...
  describe('calculateScore', () => {
    ...
    it('should assign correct bands', () => {
      ...
      const manyFindings = Array(20)
        .fill(null)
        .map(() => createFinding({ severity: 'high', category: 'encryption' }));
    ...
    it('should weight domains correctly', () => {
      ...
      expect(score.domainScores.encryption.weight).toBe(0.2);
      expect(score.domainScores.accessControl.weight).toBe(0.2);
      expect(score.domainScores.auditLogging.weight).toBe(0.15);
      expect(score.domainScores.infrastructure.weight).toBe(0.1);
      expect(score.domainScores.aiGovernance.weight).toBe(0.1);
    });
  });
});
