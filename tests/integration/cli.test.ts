...
describe('CLI: scan', () => {
  ...
  it('should output valid JSON with --json flag', async () => {
    ...
    expect(parsed.rulesEvaluated).toBe(29);
...
describe('CLI: score', () => {
  ...
  it('should output valid JSON with --json flag', async () => {
    ...
    expect(['compliant', 'needs_improvement', 'at_risk', 'critical']).toContain(parsed.band);
    expect(Object.keys(parsed.domainScores).length).toBe(6);
    expect(parsed.metadata.rulesEvaluated).toBe(29);
  });
